...
class SearchFormMixin(object):
    ...
    def filters(self, queryset):
        ...
        kwargs = {
            field_name: value for field_name, value in self.cleaned_data.items()
        ...
        for field_name, lookup in self.Meta.field_lookups.iteritems():
            ...
            if inspect.isfunction(lookup):
                processed_filters = lookup(self, queryset, value)
            ...
            if type(lookup) is tuple:
                """
                if its tuple of field_lookups, remove the field name lookup from filters and add a 
                new Q Object filter for each field lookup in the tuple then reduce the list of 
                Q objects using the OR operator
                """
                if value:
                    ...
                    q_field_lookups = [Q(**{'%s' % field_lookup: value}) for field_lookup in lookup]
                ...
                kwargs = replace_key(field_name, lookup, kwargs)
        if q_filters:
